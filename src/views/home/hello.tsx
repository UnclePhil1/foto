...
const GameSandbox: FC = () => {
  ...
  const velocityRef = useRef(0); // ADDED: Store velocity in ref for smooth updates

  // Jars (collectibles) and immunity state
  ...
  // New features: coins, power-ups, achievements, trail particles
  ...
  // Parallax / background dynamic
  const [starOffset, setStarOffset] = useState({ a: 0, b: 0, c: 0 });
  const [gradientHue, setGradientHue] = useState(270);

  // Daily challenges
  type DailyChallenge = { id: string; title: string; type: 'survive'|'collect'; target: number; progress: number; unlocked: boolean };
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        const choice = challengesPool[idx] || challengesPool[Math.floor(Math.random() * challengesPool.length)];
  ...
  const saveCoinsCollected = (c: number) => {
    try { localStorage.setItem('scrollOrDie_totalCoins', c.toString()); } catch {}
  };
  ...
  const checkAchievement = useCallback((id: string) => {
    if (!unlockedAchievements.includes(id)) {
      ...
      if (a) {
        ...
        try { localStorage.setItem('scrollOrDie_achievements', JSON.stringify(updated)); } catch {}
  ...
  const handleDeath = useCallback((playerX: number, playerYPos: number) => {
    ...
    if (todayChallenge && todayChallenge.type === 'survive') {
      try {
        const days = Math.floor(Date.now() / 86400000);
        const key = `scrollOrDie_challenge_${days}`;
        const updated = { 
  ...
  }, [score, highScore, createDeathParticles, muted, scoreMultiplier, todayChallenge]);
  ...
  // FIXED: Smooth game loop with optimized player movement
  useEffect(() => {
      ...
      setCoins(prev => {
        ...
        prev.forEach(coin => {
          ...
          if (distP < 4.5) {
            ...
            // Update daily challenge progress
            if (todayChallenge && todayChallenge.type === 'collect') {
              try {
                const days = Math.floor(Date.now() / 86400000);
                const key = `scrollOrDie_challenge_${days}`;
                const updated = { 
  ...
  }, [gameState, isImmune, difficultyMode, getDifficulty, spawnObstacle, spawnCoin, spawnPowerUp, spawnMeteor, checkCollision, checkMeteorCollision, handleDeath, playSound, createParticles, addTrailParticle, activatePowerUp, hasMagnet, hasDouble, todayChallenge]);

  // Update particles
  useEffect(() => {
    if (particles.length === 0 && trailParticles.length === 0) return;
  ...
  }, [particles.length, trailParticles.length]);

  // Keep playerYRef up to date
  ...
  }, [shieldTimer, slowmoTimer, doubleTimer, magnetTimer]);
  ...
  // FIXED: Smooth scroll handling with momentum
  const handleInput = useCallback((delta: number) => {
    if (gameState === 'start') {
    ...
    if (gameState === 'playing') {
      ...
      const danger = Math.min(1, scrollSpeed / 5);
      setDangerLevel(d => Math.min(1, d + danger * 0.3));

      // Smoother scroll force with momentum
      const scrollForce = -delta * SCROLL_SENSITIVITY * 0.08;
      
      setVelocity(v => {
        const newV = v + scrollForce;
        const clamped = Math.max(-MAX_VELOCITY, Math.min(MAX_VELOCITY, newV));
        velocityRef.current = clamped; // Update ref immediately
        return clamped;
      });
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      touchVelocity = deltaY / timeDiff;
      
      // Apply force with momentum
      const scrollForce = -touchVelocity * SCROLL_SENSITIVITY * 0.12;
      ...
      if (gameState === 'playing') {
        ...
        // Update danger level based on touch speed
        const danger = Math.min(1, Math.abs(touchVelocity) / 30);
        setDangerLevel(d => Math.min(1, d + danger * 0.2));
      }
    ...
    const handleTouchEnd = () => {
      // Apply a little momentum on touch end
      setTimeout(() => {
  ...
  const resetGame = () => {
    ...
    try {
      const lost = lostRef.current;
      if (lost) { lost.pause(); lost.currentTime = 0; }
    } catch {}
  ...
  const shareScore = () => {
    try {
      const pageUrl = typeof window !== 'undefined' ? window.location.href : 'https://scrolly.games';
      const finalScore = score * scoreMultiplier;
    ...
    >
      ...
      {gameState === 'start' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-10 p-4">
          ...
          {/* Social buttons (removed friends, kept challenges) */}
